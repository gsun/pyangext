...
def pyang_plugin_init():
    pyangext_Plugin.register_plugin(HxPlugin())
...
class HxPlugin(pyangext_PyangPlugin):
    _hx_class_name = "HxPlugin"
    __slots__ = ()
    _hx_fields = []
    _hx_methods = ["add_opts", "add_output_format", "emit", "setup_ctx", "setup_fmt", "emit_tree"]
    _hx_statics = ["pyang_plugin_init"]
    _hx_interfaces = []
    ...
    def emit(self,ctx,modules,writef):
        self.emit_tree(ctx,modules,writef)
    ...
    def emit_tree(self,ctx,modules,fd):
        _g = 0
        while (_g < len(modules)):
            module = (modules[_g] if _g >= 0 and _g < len(modules) else None)
            _g = (_g + 1)
            fd.write((Reflect.field(module,"i_modulename") if (python_Boot.hasField(module,"i_modulename")) else None))
